...
const TIMER_HELP = [
  ...
  },
  {
    ...
    title: '集中・休憩の切り替え / タブロック',
    desc: 'タイマー動作中は本棚・統計タブへの移動がロックされます。設定タブは使用できます',
    detail: '集中フェーズが終わると自動的に休憩フェーズへ移行します。休憩が終わると再び集中フェーズに戻ります。途中でストップボタンを押すとリセットできます。なお、タイマー動作中は本棚・統計タブへの移動がロックされます（設定タブは引き続き使用できます）。一時停止・停止するとロックは解除されます。',
    image: (
      <svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" width="280" height="160">
        <rect width="280" height="160" fill="#F7F7FB" rx="8" />
        {/* 集中バッジ */}
